...
npdat = np.loadtxt("data_tableWS4_RBF.txt")
data_bound = []
for i in npdat:
	S1n = i[7]
	S1p = i[8]
	...
	S2p = i[10]
	Qb = i[12]
	mass = float(round(i[1]))*8.07131806+float(round(i[0]))*7.288971064-i[3]
	isbound = False
	if (S1n>0 and S2n>0 and S1p>0 and S2p>0):
		isbound = True			
	data_bound.append({"ZA": int(round(i[0]))*1000+int(round(i[2])),"N": int(round(i[1])),"Z": int(round(i[0])),"A": int(round(i[2])),"EL": getnamebyz(int(round(i[0]))),"Ebind":i[3],"mass":mass,"S1n":S1n,"S2n":S2n,"S1p":S1p,"S2p":S2p,"Qb":Qb,"Qbn":-999999,"isbound":isbound})

np.save("data_WS4RBF.npy",data_bound)
