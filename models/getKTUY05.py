...
npdat = np.loadtxt("KTUY05_m246.dat.txt")
data_raw = []
for i in npdat:
    ZZ=int(round(i[0]))
    NN=int(round(i[1]))
    mass=i[2]
    Ebind = float(round(i[1]))*8.07131806+float(round(i[0]))*7.288971064-mass
    data_raw.append({"ZA": ZZ*1000+NN+ZZ,"N": NN,"Z": ZZ,"A": NN+ZZ,"EL": getnamebyz(ZZ),"Ebind":Ebind,"Mth":mass})
# print(data_raw)
...
np.save("data_ktuy05.npy",data_bound)
