...
def load_txt(infile):
	...
	while True:
		...
		#3i5,9f10.3,6f10.2
		(Z,N,A,e2,e3,e4,e6,e6sym,b2,b3,b4,b6,Emic,Mth,Mexp,sexp,EFLmic,MFLth) = struct.unpack("5s5s5s10s10s10s10s10s10s10s10s10s10s10s10s10s10s10s",line1)
		(Z,N,A,e2,e3,e4,e6,e6sym,b2,b3,b4,b6,Emic,Mth,Mexp,sexp,EFLmic,MFLth) = map(lambda x: x.decode('utf-8').strip(),(Z,N,A,e2,e3,e4,e6,e6sym,b2,b3,b4,b6,Emic,Mth,Mexp,sexp,EFLmic,MFLth))
		(Z,N,A) = map(lambda x: int(x),(Z,N,A))
		(e2,e3,e4,e6,e6sym,b2,b3,b4,b6,Emic,Mth,Mexp,sexp,EFLmic,MFLth) = map(lambda x: float(x) if len(x)>0 == 0 else -999999,(e2,e3,e4,e6,e6sym,b2,b3,b4,b6,Emic,Mth,Mexp,sexp,EFLmic,MFLth))
		Ebind = N*8.071431+Z*7.289034-Mth
		datafrdm.append({"ZA": Z*1000+N+Z,"N": N,"Z": Z,"A": N+Z,"EL": getnamebyz(Z),"e2": e2, "Ebind":Ebind,"Mth":Mth})
	return datafrdm

Mn_MeV =  8.071431
Mp_MeV =  7.289034
#M = N*8.07131806+Z*7.288971064-Ebind
def getdriplines():
	datafrdm = load_txt('ADNDT-59-1995-185-table.dat')
	data_bound = []
	data_bound_Qbn = []
	for i in range(len(datafrdm)):
		...
		data_bound.append({"ZA": datafrdm[i]["ZA"],"N": datafrdm[i]["N"],"Z": datafrdm[i]["Z"],"A": datafrdm[i]["A"],"EL": datafrdm[i]["EL"],"Ebind":datafrdm[i]["Ebind"],"mass":datafrdm[i]["Mth"],"S1n":S1n,"S2n":S2n,"S1p":S1p,"S2p":S2p,"Qb":Qb,"Qbn":Qbn,"e2": datafrdm[i]["e2"],"isbound":isbound})
	return data_bound
data_bound = getdriplines()
np.save("data_frdm95.npy",data_bound)
