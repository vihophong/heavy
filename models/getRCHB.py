...
def processraw(infile):
    ...
    while True:
        ...
        for i in line1:
            if checkInt(i):
                if (len(isotag)==1):
                    if (isotag[0]['idx']!=ii-1):
                        isotag = []
                    else:
                        isotag.append({'idx':ii,'val':int(i)})
                if (len(isotag)==0):
                    isotag.append({'idx':ii,'val':int(i)})

            if (flag_isotag and len(isotag)==0 and i.find('.')>=0):
                iffloat = True
                try:
                    float(i)
                except ValueError:
                    iffloat=False
                if (iffloat and (isotag2[1]['idx']==ii-1)):
                    if (len(isotag2)>2):
                        print("err")
                    # print(isotag2[0]['val'],isotag2[1]['val'],i,ii,isotag2[1]['idx'])
                    ZZ = isotag2[0]['val']-isotag2[1]['val']
                    mass = float(isotag2[1]['val'])*8.07131806+float(ZZ)*7.288971064-float(i)
                    rawdata.append({"ZA": ZZ*1000+isotag2[0]['val'],"A":isotag2[0]['val'],"N":isotag2[1]['val'],"Z":ZZ,"EL":getnamebyz(ZZ),"Ebind":float(i),"Mth":mass})
                    niso+=1
                flag_isotag = False
...
data_raw = processraw("RCHB_PC-PK1_extracttxt.txt")
...
for i in range(len(data_raw)):
    ...
    data_bound.append({"ZA": data_raw[i]["ZA"],"N": data_raw[i]["N"],"Z": data_raw[i]["Z"],"A": data_raw[i]["A"],"EL": data_raw[i]["EL"],"Ebind":data_raw[i]["Ebind"],"mass":data_raw[i]["Mth"],"S1n":S1n,"S2n":S2n,"S1p":S1p,"S2p":S2p,"Qb":Qb,"Qbn":Qbn,"isbound":isbound})


np.save("data_rchb18.npy",data_bound)
